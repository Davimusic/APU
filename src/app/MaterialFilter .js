...
const MaterialFilter = ({ data, onSelect }) => {
  ...
  const [fractionFactor, setFractionFactor] = useState(100); // Nuevo estado
  ...
  const handleFractionFactorChange = (e) => setFractionFactor(e.target.value);
  ...
  return (
    <div>
      <div>
        ...
        <input type="number" placeholder="Porcentaje de precio" value={fractionFactor} onChange={handleFractionFactorChange} /> {/* Nuevo input */}
      </div>
        ...
        {filteredData.map(({ id, empresa }) => (
          <div key={id}>
            ...
            {empresa.materiales?.map((item, index) => {
              const fraccionado = Math.floor(item.precio * (fractionFactor / 100)); // Actualizar cálculo
              let info = {
                material: item.material,
                precio: item.precio,
                precioFraccionado: fraccionado, // Precio fraccionado sin decimales
                cantidad: item['cantidad disponible'],
              ...
              return (
                <div onClick={() => onSelect(info)} key={index} style={{ marginBottom: '20px', padding: '10px', border: '1px solid black' }}>
                  ...
                  <p>Precio Fraccionado: {fraccionado}</p> {/* Mostrar precio fraccionado sin decimales */}
